...
const dom = require('./dom-320b3708.js');
...
let CalciteIcon = class extends HTMLElement {
  ...
  render() {
    ...
    return (request.h$4(request.Host, { "aria-hidden": (!semantic).toString(), "aria-label": semantic ? textLabel : null, role: semantic ? "img" : null }, request.h$4("svg", { class: {
        [CSS$1.flipRtl]: dir === "rtl" && flipRtl,
        svg: true
      }, fill: "currentColor", height: "100%", viewBox: `0 0 ${size} ${size}`, width: "100%", xmlns: "http://www.w3.org/2000/svg" }, paths.map((path) => typeof path === "string" ? (request.h$4("path", { d: path })) : (request.h$4("path", { d: path.d, opacity: "opacity" in path ? path.opacity : 1 }))))));
  }
  //--------------------------------------------------------------------------
  //
  //  Private Methods
...
let CalciteNotice$1 = class extends HTMLElement {
  ...
  updateRequestedIcon() {
    this.requestedIcon = dom.setRequestedIcon(StatusIcons, this.icon, this.color);
  }
  //--------------------------------------------------------------------------
  //
  //  Lifecycle
  ...
  render() {
    const { el } = this;
    const dir = dom.getElementDir(el);
    const closeButton = (request.h$4("button", { "aria-label": this.intlClose, class: CSS.close, onClick: this.close, ref: (el) => (this.closeButton = el) }, request.h$4("calcite-icon", { icon: "x", scale: this.scale === "l" ? "m" : "s" })));
    const hasActionEnd = dom.getSlotted(el, SLOTS.actionsEnd);
    return (request.h$4("div", { class: { [CSS.container]: true, [dom.CSS_UTILITY.rtl]: dir === "rtl" } }, this.requestedIcon ? (request.h$4("div", { class: CSS.icon }, request.h$4("calcite-icon", { icon: this.requestedIcon, scale: this.scale === "l" ? "m" : "s" }))) : null, request.h$4("div", { class: CSS.content }, request.h$4("slot", { name: SLOTS.title }), request.h$4("slot", { name: SLOTS.message }), request.h$4("slot", { name: SLOTS.link })), hasActionEnd ? (request.h$4("div", { class: CSS.actionsEnd }, request.h$4("slot", { name: SLOTS.actionsEnd }))) : null, this.dismissible ? closeButton : null));
  }
  //--------------------------------------------------------------------------
  //
  //  Public Methods
